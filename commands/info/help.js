...
function getCMD(client, message, input) {
    ...
    if (!cmd) return message.channel.send(embed.setColor('#417af6').setDescription(info).setFooter('© SF Graphics, 2020 - 2021'));
    if (cmd.name) info = `**Naam**: ${cmd.name}`
    if (cmd.aliases) info += `\n**Afkortingen**: ${cmd.aliases.map(a => `\`${a}\``).join(", ")}`
    if (cmd.description) info += `\n**Beschrijving**: ${cmd.description}`
    if (cmd.usage) info += `\n**Gebruik**: ${cmd.usage}`;
    if (cmd.timeout) info += `\n**Timeout**: ${ms(cmd.timeout)}`
    return message.channel.send(embed.setColor('#417af6').setDescription(info).setFooter(`<> = Verplicht, [] = Optionaal, © SF Graphics, 2020 - 2021`))
